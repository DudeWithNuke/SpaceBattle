...
## Дискретный координатный коллайдер
Вместо физического коллайдера Unity для placeable object используется набор дискретных трехмерных координат.
Набор таких координат образует форму, которая занимает ячейки.
